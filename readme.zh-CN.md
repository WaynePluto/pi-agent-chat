...
本插件通过内置的**官方** `@earendil-works/pi-coding-agent` SDK（**v0.84.1**）实现，而非 RPC 方式；agent 循环、工具与 LLM 调用都在扩展进程内完成，无需单独安装 Pi CLI，同时与你的 Pi 现有配置完全兼容。
...
同一时间只有一条任务线在运行，运行期间不允许切换到无关会话或新建会话——但可以以**只读预览**方式打开其他会话查看内容（顶部显示提示条，返回当前会话后才能发送消息）。主代理可以把一个任务串行委派给可见的 SDK 子会话：主代理等待，用户可在父/子会话记录之间切换查看。不同 VS Code 窗口（不同项目）之间互不影响。
...
整体上，本插件的核心思路就是简洁，并相信大模型自身的发展：agent harness 对大模型的干预越少越好。本来是不想增加 `subagent` 工具的，只是出于当前上下文窗口的限制才加了它（见下文）。
...
- 可见、串行的 SDK 子代理：内置 `subagent` 工具无需 Pi CLI 即可创建持久子会话；主代理等待，父/子会话记录均可查看，并禁止嵌套和并行委派
