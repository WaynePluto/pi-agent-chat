...
- `src/agent/subagent.ts` — 插件在 pi 自带工具之外**唯一**提供的工具 `subagent`：串行子会话协调。
...
- `src/webview/` — webview 前端（无框架，DOM 直操作），按面板拆分：`main.ts` 只做布局/接线/路由，`transcript.ts`、`composer.ts`、`sessions-view.ts`、`resources-view.ts`、`picker.ts`、`statusline.ts` 各管一块，`shell.ts`/`store.ts`/`host.ts`/`collapsible.ts`/`dom.ts` 为公共设施；`i18n.ts` 提供 zh/en 双语。
...
- 会话文件与配置完全复用 `~/.pi/agent/`（auth.json、models.json、settings、extensions、skills），与终端 Pi 可互操作；不要引入插件私有的配置副本，插件**没有自己的 VS Code 配置项**（`contributes.configuration` 为空）。常用模型同理：只读写全局 settings 的 `enabledModels`，语义对齐 CLI `/scoped-models`（显式 `provider/modelId` 列表，全选或空则清空该项）。
- **工具集（红线）**：插件只在 pi 默认的 `read`/`bash`/`edit`/`write` 之外注册 `subagent` 一个工具，且**不调用 `setActiveToolsByName()`**（自定义工具在 `_buildRuntime({ includeAllExtensionTools: true })` 时已自动激活，手动重写激活集只会静默关掉扩展工具）。`grep`/`find`/`ls` 等 SDK registry 里的其他工具、以及任何新能力，一律由 `~/.pi/agent/extensions/` 下的 pi 扩展提供（CLI 与 GUI 共享同一份），**不要再往插件里加工具**。只能靠 VS Code API 实现的工具同样不要加：它们在 CLI 侧无法提供，会让两个入口的行为不一致。
- **提示词（红线）**：插件不向 system prompt 注入任何行为引导。`subagent` 的 `description` 只陈述机制事实（串行、隔离上下文、不可嵌套），不写 `promptGuidelines`、不写「应该/不应该何时用」。防滥用、工具优先级这类策略属于用户的 `~/.pi/agent/APPEND_SYSTEM.md` 或项目 `AGENTS.md`，CLI 和 GUI 自然共享。
...
- **扩展输出走 transcript**：pi 扩展的 `ctx.ui.notify` 不弹 VS Code 通知框（单行、会截断、脱离对话），而是经 `PiRuntime.setExtensionNoticeSink()` → `ChatBridge.emit()` 变成 `status` / `error` 卡片留在聊天区，多行自动折叠。sink 必须在首次 `bindExtensions()` 之前注入（`ChatBridge` 构造函数里），且闭包捕获被绑定的那个 session，子会话/preview 的隔离由 `emit()` 的 `histories` 自动生效。扩展命令执行期间（`extensionCommandDepth > 0`）标 `scope: "command"` 置于顶层并展开，其余（后台事件里的 notify）不标 scope、收进 work block 防刷屏。未接线 sink 时仍回退原生弹窗，不静默丢消息。
...
- **资源面板的配色语法只有三态**，新增行不得引入第四种含义：常规前景色 = 已加载且生效；绿色 = 本会话用过（技能加载、工具调用、提示词调用、扩展的命令/工具跑过）；灰 + 斜体（`inactive`）= 已配置但未生效。不要再把“次要信息”用灰色表达，否则“未生效”就读不出来了。GUI 不列 `Themes`（插件只用 VS Code 主题变量，pi 主题在这里不生效）。
