...
| webview/scrollbars | `src/webview/scrollbars.ts` | 滚动条的「正在滚动」状态：document 上一个 capture 阶段的 `scroll` 监听给被滚元素加 `pi-scrolling`、静置 900ms 摘掉，样式侧据此把 `scrollbar-color` 从透明切到 slider 色。CSS 没有这个状态，所以「亮一会儿」只能由这里的延时承担；`scrollbar-color` 会继承，因此一个监听 + 根上一条规则就覆盖全部滚动容器（含后建的代码块与弹层），不需要滚动容器清单。 |
